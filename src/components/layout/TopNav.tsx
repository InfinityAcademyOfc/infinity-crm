...
export function TopNav() {
  ...
  return <header className="sticky top-0 z-50 w-full border-b bg-background/50 backdrop-blur supports-[backdrop-filter]:bg-background/50">
      <div className="container flex h-14 items-center bg-gray-950">
        {/* Título/subtítulo alinhados à esquerda apenas */}
        <div className="flex flex-col items-start justify-center flex-1">
          {moduleInfo.name && <span className="text-md font-bold">{moduleInfo.name}</span>}
          {moduleInfo.subtitle && <span className="text-xs text-muted-foreground">{moduleInfo.subtitle}</span>}
        </div>
