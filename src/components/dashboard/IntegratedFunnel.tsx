...
const IntegratedFunnel = () => {
  ...
  const getFunnelIcon = (type: FunnelType) => {
    switch (type) {
      case 'sales':
        return <Filter className="mr-2 h-4 w-4 text-blue-500" />;
      case 'ltv':
  ...
  return <Card className="shadow-md border border-border/60 bg-card/90 dark:bg-gray-800/90 backdrop-blur-sm">
      ...
      <CardContent className="bg-gray-900">
        <Tabs value={activeTab} onValueChange={value => setActiveTab(value as FunnelType)} className="space-y-4">
          ...
          {Object.keys(funnelData).map(type => <TabsContent key={type} value={type} className="space-y-2">
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={funnelData[type as FunnelType].stages} layout="vertical" margin={{
                top: 5,
                right: 30,
                left: 30,
                bottom: 5
                    ...
                    <Tooltip formatter={(value, name) => [name === 'count' ? `${value} itens` : name === 'efficiency' ? `${value}% eficiência` : `${value}% fugas`, name === 'count' ? 'Quantidade' : name === 'efficiency' ? 'Eficiência' : 'Fugas']} />
                    <Legend />
              ...
              {funnelData[type as FunnelType].stages.some(stage => stage.leakage > 30) && <div className="flex items-center p-2 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-sm">
