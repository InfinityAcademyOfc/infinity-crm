
import React, { useState } from "react";
import { DocumentItem, SpreadsheetSheet } from "../document-explorer/types";
import { Skeleton } from "@/components/ui/skeleton";

export interface SpreadsheetContentProps {
  selectedFile: DocumentItem | null;
}

const SpreadsheetContent: React.FC<SpreadsheetContentProps> = ({ selectedFile }) => {
  const [activeSheetIndex, setActiveSheetIndex] = useState(0);
  
  if (!selectedFile) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-4 text-muted-foreground">
        <p>Selecione um arquivo para visualizar ou editar</p>
      </div>
    );
  }
  
  // Get sheets or initialize with default sheet if not present
  const sheets = selectedFile.sheets || [
    {
      id: "default",
      name: "Sheet 1",
      data: Array(10).fill(Array(10).fill(''))
    }
  ];
  
  const activeSheet = sheets[activeSheetIndex] || sheets[0];

  return (
    <div className="h-full border-t">
      <div className="p-4">
        <h3 className="text-lg font-semibold mb-4">{selectedFile.name}</h3>
        
        {/* Sheet tabs */}
        {sheets.length > 1 && (
          <div className="flex mb-4 border-b">
            {sheets.map((sheet, index) => (
              <button
                key={sheet.id}
                className={`px-4 py-2 text-sm ${index === activeSheetIndex ? 'border-b-2 border-primary font-medium' : 'text-muted-foreground'}`}
                onClick={() => setActiveSheetIndex(index)}
              >
                {sheet.name}
              </button>
            ))}
          </div>
        )}
        
        {/* Spreadsheet grid */}
        <div className="grid grid-cols-10 gap-0.5">
          {Array(10).fill(0).map((_, rowIdx) => (
            <React.Fragment key={`row-${rowIdx}`}>
              {Array(10).fill(0).map((_, colIdx) => {
                // Get cell value from sheet data if available
                const cellValue = activeSheet && 
                  activeSheet.data && 
                  activeSheet.data[rowIdx] && 
                  activeSheet.data[rowIdx][colIdx];
                  
                // Display column headers (A, B, C...) or row numbers (1, 2, 3...)
                const displayValue = rowIdx === 0 ? 
                  String.fromCharCode(65 + colIdx) : 
                  colIdx === 0 ? 
                    rowIdx : 
                    cellValue || '';
                    
                return (
                  <div 
                    key={`cell-${rowIdx}-${colIdx}`}
                    className="border h-8 px-2 flex items-center text-sm"
                  >
                    {displayValue}
                  </div>
                );
              })}
            </React.Fragment>
          ))}
        </div>
      </div>
    </div>
  );
};

export default SpreadsheetContent;
